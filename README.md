...
테스트엣허 스레드 수가 증가할 수록 속도가 빨라질 것이라는 예상과 다른 현상이 관측되었다. 스레드 생성 비용이 1000개의 배열을 정렬하는 것보다 커서 이러한 현상이 관측된 것 같아 더 큰 input을 넣기 위해 만들었다.
...
앞의 실험에서는 입력 데이터의 크기가 작을 경우 스레드 생성 및 관리 오버헤드가 정렬 연산 비용보다 상대적으로 커지기 때문에, 스레드 수가 증가할수록 오히려 성능이 저하되는 현상이 관찰되었다. 
반면, 데이터 크기가 충분히 커질 경우 각 스레드가 수행하는 작업량이 증가하여 병렬 처리의 이점이 나타났으며, 이때는 CPU 코어 수에 근접한 스레드 개수에서 최적의 성능을 보였다. 
그러나 일정 수준 이상의 스레드 수에서는 최종 병합 단계가 병목으로 작용하여 성능 향상이 제한되었다.
