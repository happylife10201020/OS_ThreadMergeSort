...
P-core가 다수 존재하는 M4 칩의 특성상 멀티스레딩의 이점이 뚜렷하게 나타나며, 특히 **6~8개의 스레드를 할당했을 때 최적의 성능(Sweet Spot)**을 보입니다. 

흥미로운 점은 스레드 개수가 $2^n + 1$ 개(예: 3, 5, 9, 17개...)가 될 때마다 데이터 분할 및 병합 과정의 불균형으로 인해 성능이 눈에 띄게 하락하는 패턴이 관찰된다는 것입니다. 또한, 스레드 개수를 99개까지 극단적으로 늘리더라도 I/O 병목 및 순차 처리 구간의 한계로 인해 전체 CPU 점유율은 약 40% 미만에서 더 이상 크게 증가하지 않는 현상이 확인되었습니다.
